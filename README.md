...
The `<select>` tag has a built-in chevron (downward-pointing character), but it's inconsistent between browsers. We want to use the provided “chevron-down” icon. But how do we replace the default built-in one?? This is something we haven't learned how to do, and so you'll need to do some googling + experimentation. I want you to get a bit of practice trying to tackle “unconventional” challenges like this! If you get stuck and/or would like a hint, you'll find one on
the solution page: https://courses.joshwcomeau.com/css-for-js/03-components/19-workshop-select#select
